...
parser.add_argument('--dataset', type=str, default='soced', help='name of corpus')
parser.add_argument('--data_path', type=str, default='./soced_min_df_2', help='directory containing data')
parser.add_argument('--save_path', type=str, default='./soced_results_2', help='path to save results')
...
## Get data
# 1. vocabulary
with open(os.path.join(args.data_path, "id2word.pkl"), "rb") as infile:
    vocab = pickle.load(infile)
# 2. training data
training_set = sparse.load_npz(os.path.join(args.data_path, "bow_train.npz"))
# 3. dev set
valid = sparse.load_npz(os.path.join(args.data_path, "bow_val.npz"))
# 4. test set
test_1 = sparse.load_npz(os.path.join(args.data_path, "bow_test_h1.npz"))
test_2 = sparse.load_npz(os.path.join(args.data_path, "bow_test_h2.npz"))
...
## Get pretrained ERIC embeddings
with open(os.path.join(args.data_path, "embeddings.npy"), "rb") as infile:
    embeddings = torch.from_numpy(np.load(infile))
args.embeddings_dim = embeddings.shape[1]
